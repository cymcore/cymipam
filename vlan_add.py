...
def AddVlan(vlan_id, vlan_name, vlan_network, vlan_cidr, vlan_gateway, vlan_desc):
    con = sqlite3.connect(db_name)
    con.execute('PRAGMA foreign_keys = ON')
    cur = con.cursor()
    cur.execute('''INSERT INTO Vlan(vlan_id, vlan_name, vlan_network, vlan_cidr, vlan_gateway, vlan_desc) VALUES(?, ?, ?, ?, ?, ?)''',
                (vlan_id, vlan_name, vlan_network, vlan_cidr, vlan_gateway, vlan_desc))
    con.commit()
    con.close()
...
def main():
    ...
    if VlanExistsByVlanId(args.vlanId):
        print("vlan id already exists")
        sys.exit(1)

    if VlanExistsByVlanName(args.vlanName):
        print("vlan name already exists")
        sys.exit(1)

    AddVlan(args.vlanId, args.vlanName, args.vlanNetwork,
